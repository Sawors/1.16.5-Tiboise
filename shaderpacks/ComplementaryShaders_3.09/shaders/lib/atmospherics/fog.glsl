...
vec3 Fog1(vec3 color, float lViewPos, vec3 nViewPos, vec3 skyFogColor){
    #ifdef OVERWORLD
	#ifndef ONESEVEN
    float fog = lViewPos / far * 1.5 * (1 + rainStrength*0.3) * (10/FOG1_DISTANCE);
    ...
    #ifdef TWO
    float fog = lViewPos / far * 1.8 * (10/FOG1_DISTANCE);
    fog = 1.0 - exp(-0.1 * pow(fog, 10));
    ...
    #ifdef ONESEVEN
    float fogoneseven = lViewPos / 16 * (1.35-sunVisibility*0.35);
...
vec3 Fog2(vec3 color, float lViewPos, vec3 worldPos, vec3 skyFogColor){
		...
		float fog2 = lViewPos / pow(far, 0.25) * 0.035 * (1 + rainStrength) * (1 - sunVisibility*0.25*(1 - rainStrength)) * (32.0/(FOG2_DISTANCE + 0.01));
		fog2 = (1.0 - (exp(-50.0 * pow(fog2*0.125, 3.25) * eBS)));
		fog2 *= min(FOG2_OPACITY * (2 + rainStrength*2 - sunVisibility), 1.0);
		#if defined FOG2_ALTITUDE_MODE || defined COMPATIBILITY_MODE
			fog2 *= pow(clamp((eyeAltitude - FOG2_ALTITUDE*0.2) / FOG2_ALTITUDE, 0.0, 1.0), 2.0);
			fog2 *= 1.0 - altitudeFactor * (1 - rainStrength*0.25);
		#endif
...
vec3 startFog(vec3 color, vec3 nViewPos, float lViewPos, vec3 worldPos, float NdotU){
	...
	#ifdef FOG1
		if (isEyeInWater == 0) color.rgb = Fog1(color.rgb, length(worldPos.xz) * 1.025, nViewPos, skyFogColor);
	#endif
