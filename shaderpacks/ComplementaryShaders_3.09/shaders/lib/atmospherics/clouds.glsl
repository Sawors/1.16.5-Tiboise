...
vec4 DrawCloud(vec3 viewPos, float dither, vec3 lightCol, vec3 ambientCol, float NdotU){
	...
	float colorMultiplier = CLOUD_BRIGHTNESS * (0.5 - 0.25 * (1.0 - timeBrightness) * (1.0 - rainStrength));
	...
	float stretchFactor = 3.5;
	float coordFactor = 0.01875;

	if (NdotU > 0.0){
		vec3 wpos = normalize((gbufferModelViewInverse * vec4(viewPos, 1.0)).xyz);
		for(int i = 0; i < 6; i++) {
			if (cloud > 0.99) break;
			vec3 planeCoord = wpos * ((CLOUD_HEIGHT * 0.5 + (i + dither) * stretchFactor) / wpos.y) * 0.0085;
			vec2 coord = cameraPosition.xz * 0.00025 + planeCoord.xz;
			/**/ /**/ /**/ /**/ /**/ /**/ /**/ /**/ /**/ /**/ /**/ /**/ /**/ /**/ /**/ 
			if (coordFactor > 0.0) {
				float ang1 = (i + frametime * 0.25) * 2.391;
		...
		vec3 meSkyColor = pow(mix(lightMorning, lightEvening, mefade), vec3(4.0)) *cosS*cosS*cosS * 8 * meFactor;
		meSkyColor *= 1.0 - NdotU;
		meSkyColor *= 1.0 - rainStrength;

		vec3 cloudUpColor = lightCol * 1.55;
		cloudUpColor = mix(ambientNight*ambientNight*12.0, cloudUpColor, clamp(sunVisibility - meFactor * 8.0, 0.0, 1.0));
		cloudUpColor *= 1.0 + scattering;
		cloudUpColor += max(meSkyColor * 2.0, vec3(0.0));
		cloudcolor = mix(vec3(0.0),
		                 cloudUpColor,
						 cloudGradient * cloud);

		cloud *= 1.0 - 0.8 * rainStrength;
		cloud *= 1.0 - exp(-20.0 * NdotU);
	}
...
vec4 DrawEndCloud(vec3 viewPos, float dither, vec3 lightCol){
	...
	vec2 wind = vec2(cloudframetime * CLOUD_SPEED * 0.005,
				     sin(cloudframetime * CLOUD_SPEED * 0.05) * 0.002) * CLOUD_HEIGHT / 15.0;
