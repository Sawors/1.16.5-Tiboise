...
vec3 RoundSunMoon(vec3 nViewPos, vec3 sunColor, vec3 moonColor, float NdotU){
	...
	float sun = pow(abs(cosS), 2600.0 * isMoon + 1800.0 * (1 - isMoon)) * (1.0 - rainStrength);
...
void main(){
	...
	float NdotU = dot(normalize(viewPos.xyz), upVec);
		...
		float cloudMask = 0.0;
			...
			vec4 cloud = DrawCloud(viewPos.xyz, dither, lightCol, ambientCol, max(NdotU, 0.0));
		...
		/**/ /**/ /**/ /**/ /**/ /**/ /**/ /**/ /**/ /**/ 
			...
			#ifdef CLOUDS
				sunColor *= 1 - cloudMask;
				moonColor *= 1 - cloudMask;
			#endif
