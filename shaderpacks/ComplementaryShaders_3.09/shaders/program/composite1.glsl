...
void main(){
	...
	#ifdef OVERWORLD
		#if LIGHT_SHAFT_MODE == 1 || defined END
			if (isEyeInWater == 0) vl *= lightCol * lightCol * 0.5;
			else vl *= lightCol * 0.25;
		#else
			if (isEyeInWater == 0) {
				float lightShaftEndurance = LIGHT_SHAFT_ENDURANCE;
				if ((lightShaftEndurance < 5.40 || lightShaftEndurance > 5.60) && rainStrength < 1.0) {
					...
					vl *= pow(1.0 - NdotU, (0.75 + 1.0*2.25) * (1.0-rainStrength*rainStrength));
				}
				...
				vl *= mix(1.0, LIGHT_SHAFT_NOON_MULTIPLIER, timeBrightness);
				vl *= mix(LIGHT_SHAFT_NIGHT_MULTIPLIER * 0.5, 1.0, sunVisibility);
				vl *= mix(1.0, LIGHT_SHAFT_RAIN_MULTIPLIER, rainStrength*rainStrength);
			} else vl *= lightCol * 0.25 * LIGHT_SHAFT_UNDERWATER_MULTIPLIER  * (1.0 - rainStrength * 0.85);
		#endif
	#endif
