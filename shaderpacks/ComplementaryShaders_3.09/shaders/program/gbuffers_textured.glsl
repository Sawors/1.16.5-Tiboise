		...
		if (albedo.a > 0.0){
			...
			float metalness = 0.0;
				...
				if (normalMap == vec3(0.0)) {
					...
					float drip           = float(specularMap.b > 0.01 && specularMap.b < 0.05 && albedo.r < 0.35);
					...
					if (underWater     > 0.5) albedo.rgb = rawWaterColor.rgb * rawWaterColor.a * 2.5;
					if (water          > 0.5) albedo.rgb = waterColor.rgb * 2.5;
					if (drip           > 0.5) albedo.rgb = waterColor.rgb * 2.5;
